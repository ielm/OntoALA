...
class Sense(object):
    ...
    def resolve_variable(self, variable: str):
        result = []
        print("\tSynStruc")
        for key, val in self.synstruc.data.items():
            print(f"\t\t{key}: {val}")
        print("\tSemStruc")
        for element in self.semstruc.elements():
            print(f"\t\t{element}")

    def duplicate(self, repl: str):
        repl = repl.upper()
    ...
    def to_str(self, indent: int = 4) -> str:
        ...
        s += f"{' '*(indent)}synonyms: {self.synonyms}\n"
        s += f"{' '*(indent)}hyponyms: {self.hyponyms}"
        return s
